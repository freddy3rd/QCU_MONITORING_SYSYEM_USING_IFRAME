  ...
  const _scanner = async () =>{
    ...
    const scanner = new Instascan.Scanner({
      ...
      scanner.addListener("scan", (content) => {
        ...
        console.log("Scanner for Room1",index);
      });
      const webcam = await Instascan.Camera.getCameras();
    
      webcam.forEach(async (cam) => {
        console.log(cam.id);
        if (cam.id === "131a1524d7f6b318a41e8568cb2e9d382c6b0cec646880b4dc6d053b03b9b547") {
          if (webcam.length > 0) {
    ...
    setInterval(()=>{
        ...
        if(mediaStream){
          mediaStream.oninactive = function() {
            ...
            emailreport("QCUMS", "beni.creatives@gmail.com", "Error Report!","Room1")
          };
        }
  },1000)
