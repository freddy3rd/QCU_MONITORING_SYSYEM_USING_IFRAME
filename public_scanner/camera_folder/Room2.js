  ...
  const _scanner = async () =>{
    ...
    const scanner = new Instascan.Scanner({
      ...
      scanner.addListener("scan", (content) => {
        ...
        console.log("Scanner for Room2",index);
      });
      const webcam = await Instascan.Camera.getCameras();
    
      webcam.forEach(async (cam) => {
        console.log(cam.id);
        if (cam.id === "9da8989d705be502cce29f4c5faae384d8e75dafbef5827d87fd5ae04ab07364") {
          if (webcam.length > 0) {
    ...
    setInterval(()=>{
        ...
        if(mediaStream){
          mediaStream.oninactive = function() {
            ...
            emailreport("QCUMS", "beni.creatives@gmail.com", "Error Report!","Room2")
          };
        }
  },1000)
